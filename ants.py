            ...
            def buff(next):
                ...
                if next.ant:
                    if next.ant.is_container:
                        if next.ant.contained_ant and not next.ant.contained_ant.buffed_al:
                            next.ant.contained_ant.buffed_al = True
                            next.ant.contained_ant.damage *= 2
                    if not next.ant.buffed_al:
                        next.ant.buffed_al = True
                        next.ant.damage *= 2
                if next.exit == None:
                            return
                return buff(next.exit)
            return buff(self.place.exit)
        else:
